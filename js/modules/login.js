// ============================================
// LOTO GAMES POS - LOGIN CON PIN (VERSIÓN ROBUSTA)
// ============================================
...
async function obtenerUsuarios() {
  try {
    ...
    console.warn("Error al obtener usuarios de Supabase, usando predefinidos:", e);
  }
...
// Módulo de login (HTML inyectado en el body)
window.loginModule = () => {
  return `
...
// Variable global para el PIN ingresado
let pinIngresado = "";
        ...
        if (pinIngresado.length === 4) {
          window.verificarPIN(pinIngresado).then(result => {
            if (result.success) {
              ...
              // Remover el login del DOM
              const loginRoot = document.getElementById('loginRoot');
              if (loginRoot) loginRoot.remove();
              // Llamar a la función que carga el sistema
              if (window.cargarSistemaLogin) {
                window.cargarSistemaLogin(result.usuario);
              } else {
                alert('Login exitoso pero cargarSistemaLogin no está definida.');
              }
            } else {
