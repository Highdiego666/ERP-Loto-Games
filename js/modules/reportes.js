...
// CON IMPRESIÓN EN MINI PRINTER (74mm)
// ============================================
...
// ============================================
// IMPRIMIR CORTE EN MINI PRINTER (74mm)
// ============================================
...
window.imprimirCorteMiniPrinter = async () => {
  try {
    ...
    // ============================================
    // CONFIGURACIÓN DEL LOGO (Base64)
    // ============================================
    // PEGA AQUÍ TU CÓDIGO BASE64 DEL LOGO
    const LOGO_BASE64 = ''; // <--- PEGA TU CÓDIGO AQUÍ

    // ============================================
    // CONFIGURACIÓN DE LA IMPRESORA
    // ============================================
    const PAPER_WIDTH = 74; // Ancho en mm (74mm para mini printer)
    const FONT_SIZE = '11px';
    const TICKET_HEIGHT = '300mm'; // Altura mínima
    ...
    // Detalle de ventas (últimas 15)
    const detallesVentas = ventasHoy.slice(-15).reverse().map(v => {
    ...
    const metodosHtml = Object.entries(metodoPago).map(([metodo, total]) => {
      ...
      return `<div style="display: flex; justify-content: space-between; padding: 2px 0; font-size: 11px;">
        <span>${icono} ${metodo.charAt(0).toUpperCase() + metodo.slice(1)}</span>
        <span style="font-weight: bold;">$${total.toFixed(2)}</span>
      </div>`;
    }).join('');

    // Logo HTML (si hay logo, muestra la imagen, si no, muestra texto)
...
⚠️ TÉRMINOS DE GARANTÍA
• Garantía por reparación: 30 días
• No cubre daños por líquidos, golpes o mal uso
• Etiqueta de seguridad debe estar intacta
    ...
    win.document.write(`
      ...
      <head>
        ...
        <style>
          * { margin: 0; padding: 0; box-sizing: border-box; }
          ...
          .ticket .garantia strong { color: #222; }
          @page { 
            size: ${PAPER_WIDTH}mm ${TICKET_HEIGHT}; 
            margin: 0; 
          }
          @media print {
            body { padding: 3px; }
      ...
      <body>
        <div class="ticket">
          ...
          <div class="garantia">
            <strong>${terminosGarantia}</strong>
          </div>
          <hr>
...
window.generarReporteVentas = async (container) => {
  ...
  const inicioEl = document.getElementById('repFechaInicio');
  const finEl = document.getElementById('repFechaFin');
  if (inicioEl) inicioEl.value = inicio.toISOString().slice(0,10);
  if (finEl) finEl.value = hoy.toISOString().slice(0,10);
...
window.generarReporteInventario = async (container) => {
  ...
  const inicio = new Date();
  inicio.setDate(inicio.getDate() - 30);
  const inicioEl = document.getElementById('invFechaInicio');
  const finEl = document.getElementById('invFechaFin');
  if (inicioEl) inicioEl.value = inicio.toISOString().slice(0,10);
  if (finEl) finEl.value = hoy.toISOString().slice(0,10);
